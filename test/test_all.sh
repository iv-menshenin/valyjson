...
go test -v --run=Test_fld_FillStatements . || echo "##teamcity[testFailed name='Tests_Preparation']"
...
go test -v ./... || echo "##teamcity[testFailed name='generator.GeneratedCode']"
echo "##teamcity[testSuiteFinished name='generator.GeneratedCode']"

echo "##teamcity[testSuiteStarted name='generator.UnitTests']"
cd ../../../generator || exit 1
go test -v ./... || echo "##teamcity[testFailed name='generator.UnitTests']"
